...
| Harness sessions | `HUB/.sessions/` or `.wiki/.sessions/` | Cross-runtime OpenClaw session context |
...
"Observed tool events" means events the current harness adapter exposes. OpenClaw
has its own hook coverage, so do not promise an exact total count across all
tool classes.
...
```yaml
title: "Session Digest: openclaw:abc123"
type: session-digest
schema_version: 1
harness: "openclaw"
native_session_id: "abc123"
llm_wiki_session_id: "openclaw:abc123"
cwd: "/path/to/project"
...
raw_transcripts: false
promoted_to: []
summary: "Automated checkpoint for openclaw:abc123."
```
...
```bash
llm-wiki-session hook --harness openclaw --if-enabled
```
...
For OpenClaw, use command hooks or plugin events such as session/message/tool
execution and the compaction hook surface where available. All adapters
normalize to the same `.sessions/` files.
...
- openclaw:abc123 — Automated checkpoint summary. Digest: /abs/path/HUB/.sessions/digests/2026/06/openclaw-abc123.md
...
llm-wiki-session rehydrate --session-id openclaw:abc123
...
```bash
llm-wiki-session promote openclaw:abc123 --topic meta-llm-wiki
```
