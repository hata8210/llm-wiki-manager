...
description: >
  LLM-compiled knowledge base manager for OpenClaw. Use it to initialize, ingest,
...
You manage an LLM-compiled knowledge base. Source documents are ingested into `raw/`, then incrementally compiled into a wiki of interconnected markdown articles. OpenClaw is both the compiler and the query engine.

## OpenClaw Integration Notes

This skill is loaded as an instruction file. OpenClaw does not have specific slash commands or @-invocations. Treat any `/wiki:*` references in this skill and its references as shorthand for the equivalent natural-language request. For example, `/wiki:compile` means the user is asking you to compile the wiki.

OpenClaw's built-in tools (`read`, `write`, `edit`, `glob`, `grep`, `bash`, `webfetch`, `websearch`) map directly to the tools this skill requires.

**Permissions**: OpenClaw might sandbox file access to the project directory. The wiki hub at `~/wiki/` is external. Ensure your agent configuration allows access to `~/wiki/**` and `~/.config/llm-wiki/**`. Alternatively, use `--local` mode to keep everything in `.wiki/` inside the project.
...
4. **Dual-linking for Obsidian + OpenClaw.** Cross-references use both `[[wikilink]]` (for Obsidian graph view) and standard markdown `[text](path)` (for OpenClaw navigation) on the same line: `[[slug|Name]] ([Name](../category/slug.md))`. Bidirectional when it makes sense.
...
### Lessons Learned (ll)
Flow: Scan session for error→fix patterns, corrections, discoveries → extract structured lessons → write to `raw/notes/` with `type: lessons-learned` → optionally update relevant articles → optionally suggest agent-specific rules.
...
Multiple OpenClaw sessions can safely read and write to the same wiki simultaneously. No locks are needed.
...
Automated OpenClaw session capture uses `HUB/.sessions/`
